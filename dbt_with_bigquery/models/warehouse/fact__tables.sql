...
FROM
    ...
    INNER JOIN {{ ref('dim__EDUCATION') }} ie ON fact__tables.NAME_EDUCATION_TYPE = ie.NAME_EDUCATION_TYPE 
    INNER JOIN {{ ref('dim__HOUSING') }} ih ON fact__tables.NAME_HOUSING_TYPE = ih.NAME_HOUSING_TYPE
    INNER JOIN {{ ref('dim__INCOME') }} ii ON fact__tables.NAME_INCOME_TYPE = ii.NAME_INCOME_TYPE
    INNER JOIN {{ ref('dim__MARITAL') }} im ON fact__tables.NAME_FAMILY_STATUS = im.NAME_FAMILY_STATUS
    LEFT JOIN {{ ref('dim__JOB') }} ij ON fact__tables.OCCUPATION_TYPE = ij.OCCUPATION_TYPE
